# 資料集準備指南

## 📦 THETIS 網球資料集

### 關於 THETIS

THETIS (Tennis High-resolution Event and Tracking Identification System) 是一個公開的網球比賽資料集，包含：
- 高品質 RGB 影片
- 多種擊球動作標註
- 適合用於動作辨識研究

**GitHub 儲存庫**: https://github.com/THETIS-dataset/dataset

---

## 🔽 下載方式

### 方式 1：使用下載腳本（推薦）

```bash
# 執行下載腳本
./scripts/download_thetis.sh

# 或指定下載目錄
./scripts/download_thetis.sh ~/Downloads/my_thetis
```

### 方式 2：手動下載

```bash
# 1. 建立暫存目錄
mkdir -p ~/Downloads/thetis_rgb
cd ~/Downloads/thetis_rgb

# 2. 使用 Git sparse checkout 只下載 VIDEO_RGB
git init
git config core.sparseCheckout true
echo "VIDEO_RGB" >> .git/info/sparse-checkout

# 3. 下載資料
git remote add origin https://github.com/THETIS-dataset/dataset.git
git pull origin main
```

下載完成後，影片會在 `VIDEO_RGB/` 資料夾中。

---

## 📂 整理影片到專案

下載後需要將影片按動作類型分類到專案資料夾：

```bash
data/raw/tennis/
├── flat_service/       # 平擊發球
├── slice_service/      # 切削發球
├── smash/             # 扣殺/殺球
├── forehand_flat/     # 正手平擊
├── backhand/          # 反手擊球
├── forehand_volley/   # 正手截擊
└── backhand_volley/   # 反手截擊
```

### 整理步驟

1. **查看 THETIS 的標註檔案**
   - 檢查資料集中的 annotations 或 labels 檔案
   - 了解每個影片對應的動作類型

2. **手動分類** 或 **編寫腳本自動分類**
   ```bash
   # 範例：手動複製
   cp ~/Downloads/thetis_rgb/VIDEO_RGB/serve_001.mp4 \
      data/raw/tennis/flat_service/
   ```

3. **驗證分類**
   - 確認每個資料夾內有足夠的影片
   - 建議每類至少 10-20 個影片

---

## 🎥 其他資料來源

### YouTube 影片下載

使用 `yt-dlp` 下載網球教學影片或比賽片段：

```bash
# 安裝 yt-dlp
brew install yt-dlp  # macOS
# 或 pip install yt-dlp

# 下載影片
yt-dlp -f "best[height<=720]" \
    -o "data/raw/tennis/smash/%(title)s.%(ext)s" \
    "https://www.youtube.com/watch?v=VIDEO_ID"
```

### 自己錄製影片

- 使用手機或相機錄製擊球動作
- 建議：清晰背景、完整動作、2-5 秒長度
- 格式：MP4, AVI, MOV 皆可

---

## ✂️ 影片剪輯建議

如果影片太長，需要剪輯成多個短片段：

### 使用 FFmpeg

```bash
# 安裝 FFmpeg
brew install ffmpeg  # macOS

# 剪輯影片（從第 10 秒開始，剪 3 秒）
ffmpeg -i input.mp4 -ss 00:00:10 -t 00:00:03 -c copy output.mp4

# 批次剪輯腳本範例
for i in {1..10}; do
    start=$((i * 5))
    ffmpeg -i long_video.mp4 -ss $start -t 3 -c copy clip_$i.mp4
done
```

---

## 📊 資料集統計建議

### 最小需求
- **每個類別**: 10-20 個影片
- **總計**: 70-140 個影片
- **訓練/驗證/測試**: 自動分割為 70%/15%/15%

### 理想規模
- **每個類別**: 50+ 個影片
- **總計**: 350+ 個影片
- **多樣性**: 不同球員、角度、場地

---

## 🔄 預處理流程

資料準備完成後，執行預處理：

```bash
python src/data/preprocess_videos.py \
    --raw_dir data/raw/tennis \
    --output_dir data/processed/tennis \
    --split_ratio 0.7 0.15 0.15 \
    --target_size 224 224
```

這會自動：
- ✅ 提取 RGB frames
- ✅ Resize 到 224×224
- ✅ 自動分割為 train/val/test
- ✅ 保存為訓練可用格式

---

## 💡 小提示

- **先小後大**: 先用少量資料（每類 2-3 個影片）測試流程
- **資料增強**: 如果資料少，預處理時會自動進行資料增強
- **命名規範**: 影片檔名建議有意義（如 `forehand_01.mp4`）
- **備份**: 保留原始影片的備份

---

## 📞 需要協助？

如果在資料準備過程中遇到問題：
1. 查看 `quickstart.md` 的疑難排解章節
2. 檢查影片格式是否支援（`.mp4`, `.avi`, `.mov`）
3. 確認資料夾結構是否正確
